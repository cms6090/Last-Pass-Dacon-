...
이 파이프라인은 “마지막 Pass 예측”을 위해 몇 가지 핵심 개념을 사용합니다.
...
- 너무 크면: 오래된 이벤트가 노이즈가 되거나 피처 수가 폭증해 과적합/학습비용이 늘 수 있음

그래서 `k_prev ∈ {3,5,7,10}`처럼 후보를 두고 **OOF 유클리드 점수로 최적의 k_prev를 선택**합니다.

---

### 0.2 Pruning이란?

Pruning은 **피처(입력 변수) 수를 줄이는 과정**입니다.  
이 파이프라인에서는 AutoGluon의 feature importance를 이용해서 **중요도가 높은 피처 Top-N만 남기고 나머지는 제거**합니다.

- baseline feature set: 원래 생성된 모든 피처
- pruned feature set: importance 상위 `TOP_N`개 피처만 사용

왜 pruning을 하나?
- 피처 수가 너무 많으면 학습 시간이 길어지고, 과적합 위험이 커질 수 있음
- 중요도가 낮은 피처를 제거하면 더 단순하고 일반화가 잘 되는 모델이 나올 수 있음(항상 그렇다고 단정할 수는 없음)
...
### 0.3 OOF(Out-Of-Fold)란?

OOF는 교차검증(CV)에서 **각 샘플을 “자기 자신이 학습에 포함되지 않은 모델”로 예측한 값**을 말합니다.

- fold 1에서 valid인 샘플은 fold 1의 train 데이터로만 학습한 모델로 예측
- 모든 fold를 합치면, 전체 데이터에 대한 “누수 없는 예측”이 만들어짐

이 파이프라인에서는 end_x/end_y 각각 OOF 예측을 만든 뒤,
둘을 합쳐 **유클리드 평균거리**로 최종 점수를 계산합니다.

---

### 0.4 GroupKFold(game_id)란?

GroupKFold는 “같은 그룹이 train/valid로 쪼개지지 않게” 나누는 CV 방식입니다.

여기서는 `groups = game_id`를 사용해,
- 같은 경기(`game_id`)의 episode들이 서로 다른 fold로 섞이지 않도록 합니다.

왜 필요한가?
- 같은 경기 내에서 반복되는 패턴(전술/스타일/경기 맥락)이 학습/검증에 동시에 들어가면 점수가 과대평가될 수 있음(누수)
- 따라서 경기 단위로 묶어 더 현실적인 일반화 평가를 합니다.

---

### 0.5 Branching(분기 학습)이란?

Branching은 `result_name`(예: Successful/Unsuccessful)에 따라
**서로 다른 모델을 학습하고 예측 시 라우팅하는 방식**입니다.

- 성공 패스 데이터로만 end_x/end_y 모델 학습
- 실패 패스 데이터로만 end_x/end_y 모델 학습
- valid/test에서 `result_name`으로 “성공이면 성공 모델, 아니면 실패 모델” 사용

왜 하나?
- 성공/실패 패스는 도착점 분포가 크게 다를 가능성이 있음
- 한 모델이 두 분포를 동시에 설명하면 평균화되어 손해볼 수 있어 분리 학습을 시도

안전장치:
- 한쪽 표본이 너무 적으면(`min_side=50`) 분기 대신 전체 모델로 fallback

⚠️ 제약:
- 테스트에 `result_name`이 없으면 branching을 사용할 수 없어 자동 비활성화됩니다(코드에 구현됨).

---

### 0.6 Postprocess(후처리)란?

후처리는 모델이 낸 `(end_x, end_y)` 예측을
간단한 규칙으로 **물리적으로 보정**하는 과정입니다.
